...
class Layer:
    ...
    def setWavelength(self, wavelength):
        if self.material is not None:
            self.er = self.material.er(wavelength)
            self.ur = self.material.ur(wavelength)
            self.n = self.material.n(wavelength)
...
class LayerStack:
    def __init__(self, *layers):
        ...
        if len(layers) is 0:
            ...
            # Check that the first and last layers are actually infinite - if they are not,
            # we want to make the reflection layer the default of free space
            if(layers[0].L == 0):
                self.reflectionLayer = freeSpaceLayer
                self.internalLayer = list(layers[0:-1])
    ...
    def setWavelength(self, wavelength):
        self.reflectionLayer.setWavelength(wavelength)
        self.transmissionLayer.setWavelength(wavelength)
        for layer in self.internalLayer:
            layer.setWavelength(wavelength)
